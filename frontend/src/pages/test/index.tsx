...
function Test() {
    ...
    const usePromotionByCode = async (code: string) => {
        try {
            if (!code) {
                messageApi.error("กรุณากรอกรหัสโปรโมชั่น");
                return;
            }

            const res = await UsePromotion({ promotion_code: code }); // ส่งรหัส promotion_code

            if (res && res.status === 200) {
                const updatedPromotion = res.data.promotion; // รับข้อมูลโปรโมชั่นที่อัปเดตจาก server

                // อัพเดต use_count ใน state ของ frontend
                setPromotions((prevPromotions) =>
                    prevPromotions.map((promo) =>
                        promo.id === updatedPromotion.id
                            ? { ...promo, use_count: updatedPromotion.use_count }
                            : promo
                    )
                );

                messageApi.success("ใช้โปรโมชั่นสำเร็จ");
                setInputCode(""); // ล้าง input field
            } else {
                messageApi.error(res.error || "ไม่สามารถใช้โปรโมชั่นได้");
            }
        } catch (error) {
            ...
            <Row justify="center" style={{ marginBottom: 20 }}>
                <Col span={12}>
                    <Input
                        ...
                        onPressEnter={() => usePromotionByCode(inputCode)} // เรียกใช้ฟังก์ชัน
                        style={{
                            marginTop: "10px",
                            borderRadius: "8px",
                            padding: "10px",
                    ...
                    <Button
                        type="primary"
                        onClick={() => usePromotionByCode(inputCode)} // เรียกใช้ฟังก์ชัน
